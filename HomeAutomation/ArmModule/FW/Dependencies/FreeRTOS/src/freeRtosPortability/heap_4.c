...
uint8_t* heap_4_heapStart = ucHeap;
...
void pvPortSetTypeName(void* memory, const char* typeName8Chars) {

#ifdef MODIFIED_HEAP_4

    uint8_t* ptr = (uint8_t*)memory;

    BlockLink_t* trackedMemory = (BlockLink_t*)(ptr - heapSTRUCT_SIZE);

    strncpy(trackedMemory->typeName, typeName8Chars, sizeof(trackedMemory->typeName)-1);

    trackedMemory->typeName[sizeof(trackedMemory->typeName) - 1] = NULL;
...
bool isHeapAddress(void* address) {
    ...
    return ((uint8_t*)address >= ucHeap && (uint8_t*)address < (ucHeap + configTOTAL_HEAP_SIZE));
