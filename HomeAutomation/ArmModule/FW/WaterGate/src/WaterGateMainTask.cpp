#include <WaterGateMainTask.h>

WaterGateMainTask::WaterGateMainTask(QueueHandle_t sensorSampleQ, QueueHandle_t httpRequestQueue, 
...
WaterGateMainTask::~WaterGateMainTask() {
}

void WaterGateMainTask::handlePirIrq() {
    if (isWet()) {
...
void WaterGateMainTask::handleButtonIrq() {
    if (isWet()) {
        waterValve->silenceAlarm();
        resetState(true);
    } else {
...
void WaterGateMainTask::blueLedCallback(TimerHandle_t timer) {
...
void WaterGateMainTask::dailyChecksCallback(TimerHandle_t timer) {
    runDailyChecks = true;
}

void WaterGateMainTask::init() {
...
void WaterGateMainTask::testModeStopCallback(TimerHandle_t timer) {
    testMode = false;
}

void WaterGateMainTask::alarmLongTimerCallback(TimerHandle_t timer) {
    waterValve->soundAlarm();
    xTimerStart(alarmShortTimer, 1000/portTICK_PERIOD_MS);
}

void WaterGateMainTask::alarmShortTimerCallback(TimerHandle_t timer) {
    waterValve->silenceAlarm();
}
...
void WaterGateMainTask::setIsWet(bool isWet) {
    if (!isWet) {
        for (uint8_t i=0; i<REGISTERED_WATER_SENSOR_COUNT; i++) {
...
bool WaterGateMainTask::isWet() {
    return this->isWetValue;
}

void WaterGateMainTask::setBlueSwitchLedState(bool state, bool isFromIsr) {
...
bool WaterGateMainTask::checkHealth(char* buffer) {
...
void WaterGateMainTask::updateLcd(char* buffer, Font* largeFont, Font* medFont, Font* smallFont, Font* xSmallFont, bool isHealthy) {
    lcd->clear();
...
void WaterGateMainTask::updateLcdMainPage(char* buffer, Font* largeFont, Font* medFont, Font* smallFont, bool isHealthy) {
    uint8_t spacing = 1;
...
char* WaterGateMainTask::getOverallStatusMessage() {
    return "*******";  // this method must free memory
}

void WaterGateMainTask::updateLcdSensorList(char* buffer, Font* font, Font* smallFont, uint8_t pageNumber) {
...
bool WaterGateMainTask::waitForXbeeToFormNetwork() {
    uint8_t count = 0;
...
WaterSensorSample* WaterGateMainTask::getRegisteredWaterSensor(uint64_t address) {
    for (uint8_t i=0; i<REGISTERED_WATER_SENSOR_COUNT; i++) {
        WaterSensorSample* registeredSample = &this->registeredWaterSensors[i];
...
char* WaterGateMainTask::getWetSensorName() {
    for (uint8_t i=0; i<REGISTERED_WATER_SENSOR_COUNT; i++) {
        WaterSensorSample* registeredSample = &this->registeredWaterSensors[i];
...
WaterSensorSample* WaterGateMainTask::updateWaterSensorRegistration(WaterSensorSample* sample) {
...
WaterSensorSample* WaterGateMainTask::getSensorWithLowestBattery() {
...
WaterSensorSample* WaterGateMainTask::getSensorWithLowestRssi() {
    float weakestRssi = 0;
    WaterSensorSample* sensor = NULL;
    for (uint8_t i=0; i<REGISTERED_WATER_SENSOR_COUNT; i++) {
        WaterSensorSample* s = &this->registeredWaterSensors[i];
...
WaterSensorSample* WaterGateMainTask::getSensorWithLongestSleep() {
...
uint8_t WaterGateMainTask::getActiveWaterSensorCount() {
    uint8_t count = 0;
    for (uint8_t i=0; i<REGISTERED_WATER_SENSOR_COUNT; i++) {
        WaterSensorSample* s = &this->registeredWaterSensors[i];
...
bool WaterGateMainTask::createSpecializedHttpResponse (HttpPacket* packet, char* resource) {
...
void WaterGateMainTask::createSpecializedHttpStatusResponse(HttpPacket* packet, Zstring* msg, char* buffer) {
...
void WaterGateMainTask::resetState(bool fromIsr) {
    this->waterValve->silenceAlarm();
    if (!testMode) {
...
void WaterGateMainTask::createHttpResetResponse(HttpPacket* packet, char* resource) {
...
void WaterGateMainTask::createHttpTestModeResponse(HttpPacket* packet) {
...
void WaterGateMainTask::createHttpOverrideResponse(HttpPacket* packet, char* resource) {
...
void WaterGateMainTask::createSpecializedRootUsageResponse(Zstring* msg) {
